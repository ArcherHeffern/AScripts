    ...
    while chunk := input_file.read(CHUNK_SIZE):
        ...
        while i != len(chunk):
            ...
            if c in "}])":
                indentation -= 1
                if indentation < 0:
                    print(f"Parenthesis are not balanced at {line}:{char} (line:character)")
                    indentation = 0
                    ok = False
                    if strict:
                        return False
        ...
        if indentation != 0:
            ...
            print("Indentation should be 0 at end. Parenthesis are not balanced!", file=stderr)
            return False
    return ok
    ...
    for input_file in options.input_files:
        if options.in_place:
            ...
            if Path(output_file).exists() and not options.force_overwrite:
                print(f"Output file '{output_file}' exists. To force overwrites use the -f|-force flag. Skipping...", file=stderr)
                continue
